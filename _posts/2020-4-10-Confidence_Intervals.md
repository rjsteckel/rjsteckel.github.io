---
layout: post
title: Confidence Interval for proportions
tags: [Probability, Confidence Intervals]
---

Proportions occur in many places.

A common inference method for a proportion, from a frequentist viewpoint, is estimating confidence intervals. For a frequentist, the parameters are fixed and the data varies. The dataset you're anlayizing now is just one possible dataset of many that explain the phnonomen that you're interested in. Governing this phenomeon is some parameter that you like to infer. You may never know the true value, but you can learn things about it.

For a proportion, you have a count ($x$) of the number of times the thing you're interested in occured out of the possible times it could have happened ($n$). This is what the binomial distribution is for. One estimate for the proportion is simply, $\frac{x}{n}$.
...
$$
P \{ \lvert \frac{x}{n} - p \rvert < α \} = P \{ \lvert x - np \rvert < nα \}
$$
